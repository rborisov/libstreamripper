 ...
 *****************************************************************************/
error_code
cbuf2_init(CBUF2 * cbuf2, int content_type, int have_relay, unsigned long chunk_size, unsigned long num_chunks)
{
...
void cbuf2_destroy(CBUF2 * cbuf2)
{
	if (cbuf2->buf) {
		free(cbuf2->buf);
		cbuf2->buf = NULL;
...
static error_code cbuf2_insert(CBUF2 * cbuf2, const char *data, u_long count)
{
	u_long ft;
	u_long write_idx;
...
static void cbuf2_advance_metadata_list(RIP_MANAGER_INFO * rmi, CBUF2 * cbuf2)
{
	...
	list_for_each_prev(p, &(cbuf2->metadata_list)) {
		...
		if (metadata_pos >= cbuf2->base_idx && metadata_pos < cbuf2->base_idx + frag1) {
			/* Nothing to do */
...
static void cbuf2_advance_relay_list(RIP_MANAGER_INFO * rmi, CBUF2 * cbuf2, u_long count)
{
...
error_code cbuf2_fastforward(CBUF2 * cbuf2, u_long count)
{
	threadlib_waitfor_sem(&cbuf2->cbuf_sem);
	if (cbuf2->item_count < count) {
		threadlib_signal_sem(&cbuf2->cbuf_sem);
...
error_code cbuf2_peek(CBUF2 * cbuf2, char *data, u_long count)
{
	u_long frag1, frag2;
...
cbuf2_insert_chunk(RIP_MANAGER_INFO * rmi,
		   CBUF2 * cbuf2, const char *data, u_long count, int content_type, TRACK_INFO * ti)
{
	LIST this_page_list;
	int chunk_no;
	error_code rc;
	...
	if (content_type == CONTENT_TYPE_OGG) {
		...
		ml->m_chunk = chunk_no;
		memcpy(ml->m_composed_metadata, ti->composed_metadata, MAX_METADATA_LEN + 1);
...
static error_code cbuf2_extract_relay_ogg(CBUF2 * cbuf2, RELAY_LIST * ptr)
{
	if (ptr->m_header_buf_ptr) {
		...
		if (remaining > ptr->m_buffer_size) {
			memcpy(ptr->m_buffer, &ptr->m_header_buf_ptr[ptr->m_header_buf_off], ptr->m_buffer_size);
			ptr->m_header_buf_off += ptr->m_buffer_size;
			ptr->m_left_to_send = ptr->m_buffer_size;
		} else {
			memcpy(ptr->m_buffer, &ptr->m_header_buf_ptr[ptr->m_header_buf_off], remaining);
	...
	*len = cbuf2->chunk_size;
	...
	if (icy_metadata) {
		...
		if (have_metadata) {
			...
			memcpy(&data[cbuf2->chunk_size], tmp->m_composed_metadata, metadata_len);
			*len += metadata_len;
		} else {
...
void cbuf2_debug_lists(RIP_MANAGER_INFO * rmi, CBUF2 * cbuf2)
{
	LIST *p;
	...
	if (rmi->http_info.content_type != CONTENT_TYPE_OGG) {
		...
		list_for_each(p, &(cbuf2->metadata_list)) {
			...
			debug_printf("%ld: %d [%d]%s\n",
				     metadata_pos,
				     meta_tmp->m_chunk,
				     (int)meta_tmp->m_composed_metadata[0], &meta_tmp->m_composed_metadata[1]);
		}
		...
		list_for_each(p, &(cbuf2->ogg_page_list)) {
			...
			debug_printf("ogg page = %02x [%ld, %ld, %ld]\n",
				     ...
				     ogg_tmp->m_page_len, cbuf2_add(cbuf2, ogg_tmp->m_page_start, ogg_tmp->m_page_len));
		}
		if (cbuf2->song_page) {
			debug_printf("%ld(%ld): Song Page\n", cbuf2->song_page->m_page_start, cbuf2->song_page_done);
		} else {
...
error_code
cbuf2_ogg_peek_song(CBUF2 * cbuf2, char *out_buf, unsigned long buf_size, unsigned long *amt_filled, int *eos)
{
	error_code ret;
	...
	*eos = 0;
	...
	if (!cbuf2->song_page) {
		if (cbuf2->ogg_page_list.next) {
			cbuf2->song_page = list_entry(cbuf2->ogg_page_list.next, OGG_PAGE_LIST, m_list);
	...
	if (cbuf2->song_page_done == cbuf2->song_page->m_page_len) {
		if (cbuf2->song_page->m_list.next == &cbuf2->ogg_page_list) {
			...
			cbuf2->song_page = list_entry(cbuf2->song_page->m_list.next, OGG_PAGE_LIST, m_list);
			cbuf2->song_page_done = 0;
		}
	}
...
u_long cbuf2_get_free(CBUF2 * cbuf2)
{
...
error_code cbuf2_peek_rgn(CBUF2 * cbuf2, char *out_buf, u_long start_offset, u_long length)
{
	u_long sidx;
...
error_code cbuf2_init_relay_entry(CBUF2 * cbuf2, RELAY_LIST * ptr, u_long burst_request)
{
	...
	if (cbuf2->content_type == CONTENT_TYPE_OGG) {
		...
		list_for_each(p, &(cbuf2->ogg_page_list)) {
			...
			debug_printf("Computing cbuf2_offset: "
				     ...
				     cbuf2->base_idx, tmp->m_page_start, page_offset, ptr->m_header_buf_ptr);
	...
	debug_printf("cbuf2_init_relay_entry: %d %d %d %d\n",
		     ptr->m_cbuf_offset, cbuf2->base_idx, cbuf2->item_count, cbuf2->size);
