...
static void apply_padding(DECODE_STRUCT * ds, unsigned long silsplit,
			  long padding1, long padding2, u_long * pos1, u_long * pos2);
...
static enum mad_flow filter(void *data, struct mad_stream const *ms, struct mad_frame *frame);
...
findsep_silence_2(const char *mpgbuf,
		  long mpgsize,
		  long len_to_sw,
		  long searchwindow, long silence_length, long padding1, long padding2, u_long * pos1, u_long * pos2)
{
	DECODE_STRUCT ds;
	...
	/* Run decoder */
	mad_decoder_init(&decoder, &ds, input /* input */ ,
			 header /* header */ ,
			 filter /* filter */ ,
			 output /* output */ ,
			 error /* error */ ,
	...
	apply_padding(&ds, ds.min_maxvolume_buffer[bestsil].pos, padding1, padding2, pos1, pos2);
...
static void
apply_padding(DECODE_STRUCT * ds, unsigned long silsplit, long padding1, long padding2, u_long * pos1, u_long * pos2)
{
	...
	debug_printf("pos1, pos2 = %d,%d (%d) (%02x%02x)\n",
		     *pos1, *pos2, *pos1 - *pos2, ds->mpgbuf[*pos2], ds->mpgbuf[*pos2 + 1]);
}
...
static enum mad_flow input(void *data, struct mad_stream *ms)
{
	...
	debug_printf("%p | %p | %p |< %p | %p >| %d\n",
		     ds->mpgbuf, ds->mpgpos, &(ds->mpgbuf[ds->mpgpos]), ms->this_frame, ms->next_frame, frameoffset);

	mad_stream_buffer(ms, (const unsigned char *)
			  (ds->mpgbuf + ds->mpgpos) - frameoffset, READSIZE);
	ds->mpgpos += READSIZE - frameoffset;

	return MAD_FLOW_CONTINUE;
}

static void propagate_max_value(unsigned short *max_buffer, unsigned long node_pos, int depth)
{
...
static void insert_value(DECODE_STRUCT * ds, unsigned short vol, unsigned long pos)
{
	int i;
	...
	for (i = 1; i < ds->maxvolume_buffer_depth; ++i) {
		unsigned long index = ds->maxvolume_buffer_offs + current_pos + (pos % current_pos);
...
static enum mad_flow filter(void *data, struct mad_stream const *ms, struct mad_frame *frame)
{
	...
	debug_printf("FILTER: %p (%02x%02x) | %p\n",
		     ms->this_frame, ms->this_frame[0], ms->this_frame[1], ms->next_frame);
#endif
...
static enum mad_flow output(void *data, struct mad_header const *header, struct mad_pcm *pcm)
{
	...
	if (ds->pcmpos > ds->len_to_sw_start_samp && ds->pcmpos < ds->len_to_sw_end_samp) {
		debug_printf("* %d\n", ds->pcmpos);
	} else {
		debug_printf("- %d\n", ds->pcmpos);
	}
...
static unsigned long next_power_of_two(long value)
{
	long result = 1;
	while (result < value)
		result *= 2;
...
static void init_maxvolume_buffer(DECODE_STRUCT * ds)
{
	...
	ds->max_search_depth = depth - (int)ceil(log(10 * (ds->samplerate / 1000.0)) / log(2)) - 1;

	/* Unless the user specified silence-length is lower. */
	if (ds->max_search_depth < 0)
		ds->max_search_depth = 0;

	ds->maxvolume_buffer = (unsigned short *)calloc(buffer_offset + ds->silence_samples, sizeof(unsigned short));
...
mad_flow header(void *data, struct mad_header const *pheader)
{
	...
	if (!ds->samplerate) {
		...
		ds->silence_samples = next_power_of_two(ds->silence_ms * (ds->samplerate / 1000));
