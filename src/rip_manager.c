...
error_code rip_manager_start(RIP_MANAGER_INFO ** rmip, STREAM_PREFS * prefs, RIP_MANAGER_CALLBACK status_callback)
{
	return rip_manager_start_with_data(rmip, prefs, status_callback, NULL);
}

/* Create a RMI structure and start the ripping thread */
error_code
rip_manager_start_with_data(RIP_MANAGER_INFO ** rmip,
			    STREAM_PREFS * prefs, RIP_MANAGER_CALLBACK status_callback, void *cb_data)
{
...
void *rip_manager_callback_data(RIP_MANAGER_INFO * rmi)
{
	return rmi->callback_data;
}
 ...
 * Private functions
 *****************************************************************************/
static void post_error(RIP_MANAGER_INFO * rmi, error_code err)
{
	ERROR_INFO err_info;
...
static void compose_console_string(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti)
{
	mchar console_string[SR_MAX_PATH];
	msnprintf(console_string, SR_MAX_PATH, m_S m_(" - ") m_S, ti->artist, ti->title);
	string_from_gstring(rmi, rmi->filename, SR_MAX_PATH, console_string, CODESET_LOCALE);
}
...
error_code rip_manager_end_track(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti)
{
	...
	if (rmi->write_data) {
		filelib_end(rmi, ti, rmi->prefs->overwrite, GET_TRUNCATE_DUPS(rmi->prefs->flags), mfullpath);
	}
	post_status(rmi, 0);
...
char *client_relay_header_generate(RIP_MANAGER_INFO * rmi, int icy_meta_support)
{
	...
	ret = http_construct_sc_response(&rmi->http_info, headbuf, MAX_HEADER_LEN, icy_meta_support);
	if (ret != SR_SUCCESS) {
		headbuf[0] = 0;
	}
...
static void debug_ripthread(RIP_MANAGER_INFO * rmi)
{
...
static int create_pls_file(RIP_MANAGER_INFO * rmi)
{
	FILE *fid;
	...
	if (NULL == fid) {
		...
		fprintf(fid, "File1=http://%s:%d\n", rmi->prefs->relay_ip, rmi->prefs->relay_port);
		fclose(fid);
	}
...
static error_code start_ripping(RIP_MANAGER_INFO * rmi)
{
	...
	ret = http_sc_connect(rmi, &rmi->stream_sock, prefs->url,
			      pproxy, &rmi->http_info, prefs->useragent, prefs->if_name);
	if (ret != SR_SUCCESS) {
	...
	ret = filelib_init
	    (rmi,
	     ...
	     GET_SEPARATE_DIRS(rmi->prefs->flags), GET_DATE_STAMP(rmi->prefs->flags), rmi->http_info.icy_name);
	if (ret != SR_SUCCESS) {
	...
	if (GET_MAKE_RELAY(rmi->prefs->flags)) {
		...
		ret = relaylib_start(rmi,
				     ...
				     rmi->prefs->relay_ip, rmi->http_info.meta_interval != NO_META_INTERVAL);
		if (ret != SR_SUCCESS) {
			goto RETURN_ERR;
		}
