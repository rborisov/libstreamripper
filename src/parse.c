...
static int sr_regcomp(Parse_Rule * pr, mchar * rule_string, int cflags)
{
...
#if !defined (USE_GLIB_REGEX)
static int mregexec(const regex_t * preg, const mchar * string, size_t nmatch, regmatch_t pmatch[], int eflags)
{
	/* GCS FIX: Use PCRE for utf8 */
...
static int compile_rule(Parse_Rule * pr, mchar * rule_string)
{
#if defined (USE_GLIB_REGEX)
	GError *error = NULL;

	pr->reg = g_regex_new(rule_string, 0, 0, &error);
	if (error) {
...
static int parse_flags(Parse_Rule * pr, char *flags)
{
	char flag1;
...
static char *parse_escaped_string(char *outbuf, char *inbuf)
{
	int escaped = 0;
