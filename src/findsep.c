		...
		long searchwindow, long silence_length, long padding1, long padding2, u_long * pos1, u_long * pos2)
{
	...
	for (i = 0; i < NUM_SILTRACKERS; i++) {
		debug_printf("SILT: %2d/%8g, pcm=%4d, found=%d, insil=%d\n",
			     ...
			     ds.siltrackers[i].foundsil, ds.siltrackers[i].insilencecount);
...
static void
apply_padding(DECODE_STRUCT * ds, unsigned long silstart, long padding1, long padding2, u_long * pos1, u_long * pos2)
{
...
error_code find_bitrate(unsigned long *bitrate, const char *mpgbuf, long mpgsize)
{
	...
	gbs.mpgsize = mpgsize;
	gbs.bitrate = 0;
	mad_decoder_init(&decoder, &gbs, input_get_bitrate /* input */ ,
