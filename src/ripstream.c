 ...
 * Private functions
 *****************************************************************************/
static error_code find_sep(RIP_MANAGER_INFO * rmi, u_long * pos1, u_long * pos2);
static error_code start_track_mp3(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti);
static error_code end_track_mp3(RIP_MANAGER_INFO * rmi, u_long pos1, u_long pos2, TRACK_INFO * ti);
static error_code end_track_ogg(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti);
static void compute_cbuf2_size(RIP_MANAGER_INFO * rmi, SPLITPOINT_OPTIONS * sp_opt, int bitrate, int meta_interval);
...
static int ripstream_recvall(RIP_MANAGER_INFO * rmi, char *buffer, int size);
static error_code get_track_from_metadata(RIP_MANAGER_INFO * rmi, int size, char *newtrack);
static error_code get_stream_data(RIP_MANAGER_INFO * rmi, char *data_buf, char *track_buf);
...
static BOOL is_track_changed(RIP_MANAGER_INFO * rmi)
{
...
static void debug_track_info(TRACK_INFO * ti, char *tag)
{
	debug_mprintf(m_("----- TRACK_INFO ") m_s m_("\n")
		      ...
		      ti->have_track_info,
		      ti->raw_metadata,
		      ti->artist, ti->title, ti->album, ti->track_p, ti->track_a, ti->year, ti->save_track);
}
...
static void copy_track_info(TRACK_INFO * dest, TRACK_INFO * src)
{
...
static error_code ripstream_rip_ogg(RIP_MANAGER_INFO * rmi)
{
	...
	if (rmi->ripstream_first_time_through) {
		...
		ret = cbuf2_init(&rmi->cbuf2, rmi->http_info.content_type,
				 GET_MAKE_RELAY(rmi->prefs->flags), rmi->getbuffer_size, rmi->cbuf2_size);
		if (ret != SR_SUCCESS)
			return ret;
	...
	ret = cbuf2_insert_chunk(rmi, &rmi->cbuf2,
				 rmi->getbuffer, rmi->getbuffer_size, rmi->http_info.content_type, &rmi->current_track);
	if (ret != SR_SUCCESS) {
	...
	if (rmi->ogg_have_track) {
		do {
			...
			ret = cbuf2_ogg_peek_song(&rmi->cbuf2, rmi->getbuffer,
						  rmi->getbuffer_size, &amt_filled, &got_eos);
...
static error_code ripstream_rip_mp3(RIP_MANAGER_INFO * rmi)
{
	...
	if (rmi->ripstream_first_time_through) {
		...
		compute_cbuf2_size(rmi, &rmi->prefs->sp_opt, rmi->bitrate, rmi->getbuffer_size);
		ret = cbuf2_init(&rmi->cbuf2, rmi->http_info.content_type,
				 GET_MAKE_RELAY(rmi->prefs->flags), rmi->getbuffer_size, rmi->cbuf2_size);
		if (ret != SR_SUCCESS)
			return ret;
	...
	if (rmi->current_track.track_p[0]) {
		...
		msnprintf(rmi->current_track.track_a, MAX_HEADER_LEN, m_("%d"), rmi->track_count + 1);
	}
	...
	if (cbuf2_get_free(&rmi->cbuf2) < rmi->getbuffer_size) {
		...
		debug_printf("cbuf2_get_free < getbuffer_size\n");
		extract_size = rmi->getbuffer_size - cbuf2_get_free(&rmi->cbuf2);
		ret = cbuf2_extract(rmi, &rmi->cbuf2, rmi->getbuffer, extract_size, &curr_song);
		if (ret != SR_SUCCESS) {
		...
		if (curr_song < extract_size) {
			...
			ret = rip_manager_put_data(rmi, &rmi->getbuffer[curr_song], curr_song_bytes);
			if (ret != SR_SUCCESS) {
		...
		*pos2 = rw_start + sw_sil;
		...
		if (sp_opt->xs == 2) {
			ret = findsep_silence_2(buf,
						...
						sp_opt->xs_padding_1, sp_opt->xs_padding_2, pos1, pos2);
		} else {
...
static error_code end_track_mp3(RIP_MANAGER_INFO * rmi, u_long pos1, u_long pos2, TRACK_INFO * ti)
{
	...
	if (GET_ADD_ID3V1(rmi->prefs->flags)) {
		...
		string_from_gstring(rmi, id3v1.artist, sizeof(id3v1.artist), ti->artist, CODESET_ID3);
		string_from_gstring(rmi, id3v1.songtitle, sizeof(id3v1.songtitle), ti->title, CODESET_ID3);
		string_from_gstring(rmi, id3v1.album, sizeof(id3v1.album), ti->album, CODESET_ID3);
		string_from_gstring(rmi, id3v1.year, sizeof(id3v1.year), ti->year, CODESET_ID3);
...
static error_code write_id3v2_frame(RIP_MANAGER_INFO * rmi, char *tag_name, mchar * data, int charset, int *sent)
{
	int ret;
	int rc;
...
static error_code start_track_mp3(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti)
{
	...
	if (GET_ADD_ID3V2(rmi->prefs->flags)) {
		...
		/* Encoded by */
		ret = write_id3v2_frame(rmi, "TENC", m_("Ripped with Streamripper"), id3_charset, &sent);
...
static int ms_to_blocks(int ms, int bitrate, int round_up)
{
...
static void compute_cbuf2_size(RIP_MANAGER_INFO * rmi, SPLITPOINT_OPTIONS * sp_opt, int bitrate, int meta_interval)
{
	...
	mic_to_sw_start = -mi_to_mic + offset - ms_to_bytes(xs_search_window_1, bitrate);
	mic_to_sw_end = -mi_to_mic + offset + ms_to_bytes(xs_search_window_2, bitrate);
	...
	debug_printf("CBUF2 (BLOCKS): %d:%d -> %d\n", mic_to_cb_start, mic_to_cb_end, rmi->cbuf2_size);
...
{
	int ret;
	ret = socklib_recvall(rmi, &rmi->stream_sock, buffer, size, rmi->prefs->timeout);
	if (ret >= 0 && ret != size) {
