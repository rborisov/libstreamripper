 ...
 * Function prototypes
 *****************************************************************************/
static char *make_auth_header(const char *header_name, const char *username, const char *password);
static char *b64enc(const char *buf, int size);
static error_code http_get_pls(RIP_MANAGER_INFO * rmi, HSOCKET * sock, SR_HTTP_HEADER * info);
static error_code http_get_m3u(RIP_MANAGER_INFO * rmi, HSOCKET * sock, SR_HTTP_HEADER * info);
static error_code http_get_sc_header(RIP_MANAGER_INFO * rmi, const char *url, HSOCKET * sock, SR_HTTP_HEADER * info);
		...
		SR_HTTP_HEADER * info, char *useragent, char *if_name)
{
	...
	while (1) {
		...
		ret = socklib_open(sock, url_info.host, url_info.port, if_name, rmi->prefs->timeout);
		if (ret != SR_SUCCESS) {
			return ret;
		}
		...
		if (*info->http_location) {
			...
			return http_sc_connect(rmi, sock, info->http_location, proxyurl, info, useragent, if_name);
		} else {
			break;
		}
	}
...
static void unescape_pct_encoding(char *s)
{
	char *d = s;
	char c[3];
...
error_code http_construct_sc_request(const char *url, const char *proxyurl, char *buffer, char *useragent)
{
	...
	snprintf(buffer, MAX_HEADER_LEN + MAX_HOST_LEN + SR_MAX_PATH,
		 ...
		 "Host: %s:%d\r\n"
		 "User-Agent: %s\r\n"
		 "Icy-MetaData:1\r\n", myurl, ui.host, ui.port, useragent[0] ? useragent : "Streamripper/1.x");
#endif
	...
	snprintf(buffer, MAX_HEADER_LEN + MAX_HOST_LEN + SR_MAX_PATH,
		 ...
		 "Icy-Metadata: 1\r\n"
		 "Connection: close\r\n"
		 "Host: %s:%d\r\n", myurl, useragent[0] ? useragent : "Streamripper/1.x", ui.host, ui.port);
...
error_code http_parse_sc_header(const char *url, char *header, SR_HTTP_HEADER * info)
{
	...
	extract_header_value(header, info->http_location, "Location:", sizeof(info->http_location));
	extract_header_value(header, info->server, "Server:", sizeof(info->server));
	rc = extract_header_value(header, info->icy_name, "icy-name:", sizeof(info->icy_name));
	if (rc == 0) {
		/* Icecast 2.0.1 */
		rc = extract_header_value(header, info->icy_name, "ice-name:", sizeof(info->icy_name));
	}
	info->have_icy_name = rc;
	extract_header_value(header, info->icy_url, "icy-url:", sizeof(info->icy_url));
	...
	rc = extract_header_value(header, stempbr, "Content-Type:", sizeof(stempbr));
	if (rc == 0) {
		rc = extract_header_value(header, stempbr, "content-type:", sizeof(stempbr));
	}
	if (rc == 0) {
	...
	// Check for Icecast 2
		...
		if (info->content_type == CONTENT_TYPE_MP3
		    && content_type_by_url != CONTENT_TYPE_UNKNOWN && content_type_by_url != CONTENT_TYPE_OGG) {
	...
	// Check for Icecast 1
		...
		// icecast 1.x headers.
		extract_header_value(header, info->icy_url, "x-audiocast-server-url:", sizeof(info->icy_url));
		rc = extract_header_value(header, info->icy_name, "x-audiocast-name:", sizeof(info->icy_name));
		info->have_icy_name |= rc;
		extract_header_value(header, info->icy_genre, "x-audiocast-genre:", sizeof(info->icy_genre));
		rc = extract_header_value(header, stempbr, "x-audiocast-bitrate:", sizeof(stempbr));
		if (rc) {
			info->icy_bitrate = atoi(stempbr);
		}
	}
...
static error_code http_get_pls(RIP_MANAGER_INFO * rmi, HSOCKET * sock, SR_HTTP_HEADER * info)
{
	...
	for (s = 1; s <= 99; s++) {
		...
		if (!extract_header_value(buf, location_buf, buf1, sizeof(location_buf))) {
			break;
		}
		if (s == 1) {
...
static char *b64enc(const char *inbuf, int size)
{
