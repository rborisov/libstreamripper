...
static error_code try_port(RELAYLIB_INFO * rli, u_short port, char *if_name, char *relay_ip);
...
static void destroy_all_hostsocks(RIP_MANAGER_INFO * rmi)
{
	RELAY_LIST *ptr;
...
relaylib_start(RIP_MANAGER_INFO * rmi,
	       ...
	       u_short * port_used, char *if_name, int max_connections, char *relay_ip, int have_metadata)
{
	int ret;
#ifdef WIN32
	WSADATA wsd;
...
static error_code relaylib_start_threads(RIP_MANAGER_INFO * rmi)
{
	...
	ret = threadlib_beginthread(&rli->m_hthread_accept, thread_accept, (void *)rmi);
	if (ret != SR_SUCCESS)
		return ret;
	rli->m_running_accept = TRUE;

	ret = threadlib_beginthread(&rli->m_hthread_send, thread_send, (void *)rmi);
	if (ret != SR_SUCCESS)
		return ret;
	...
	while (rli->m_running) {
		...
		while (rli->m_listensock != SOCKET_ERROR && rli->m_running) {
			...
			if (ret == 1) {
				...
				threadlib_signal_sem(&rmi->relay_list_sem);
				if (prefs->max_connections > 0
				    && num_connected >= (unsigned long)prefs->max_connections) {
					continue;
				}
...
static BOOL send_to_relay(RIP_MANAGER_INFO * rmi, RELAY_LIST * ptr)
{
	int ret;
	int err_errno;
	BOOL good = TRUE;
	...
	while (!done) {
		...
		ret = send(ptr->m_sock, ptr->m_buffer + ptr->m_offset, ptr->m_left_to_send, 0);
