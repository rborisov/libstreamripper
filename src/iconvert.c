...
int iconvert(const char *fromcode, const char *tocode, const char *from, size_t fromlen, char **to, size_t * tolen)
{
	int ret = 0;
	iconv_t cd1, cd2;
	char *ib;
	...
	if (!strchr("Uu", tocode[0]) ||
	    !strchr("Tt", tocode[1]) ||
	    !strchr("Ff", tocode[2]) || tocode[3] != '-' || tocode[4] != '8' || tocode[5] != '\0') {
		char *tocode1;
	...
	for (;;) {
		...
		assert((!k && !ibl) ||
		       ...
		       (k == (size_t) (-1) && (errno == EILSEQ || errno == EINVAL) && ibl));
		if (!ibl)
			break;
		if (obl < 6) {
			/* Enlarge the buffer */
	...
	while (ibl) {
		...
		assert((k != (size_t) (-1) && !ibl) ||
		       (k == (size_t) (-1) && errno == E2BIG && ibl) || (k == (size_t) (-1) && errno == EILSEQ && ibl));
		if (ibl && !(k == (size_t) (-1) && errno == E2BIG)) {
			...
			assert((!k && !tbl) || (k == (size_t) (-1) && errno == EILSEQ && tbl));
	...
	while (ibl) {
		k = iconv(cd2, &ib, &ibl, &ob, &obl);
		assert((k != (size_t) (-1) && !ibl) || (k == (size_t) (-1) && errno == EILSEQ && ibl));
		if (k && !ret)
			ret = 1;
		if (ibl && !(k == (size_t) (-1) && errno == E2BIG)) {
			/* Replace one character */
