...
static void prefs_set_stream_prefs_keyfile(STREAM_PREFS * prefs, STREAM_PREFS * default_prefs, char *group);
...
int prefs_load(void)
{
	...
	if (rc) {
		...
		case PREFS_VERSION_1_63_BETA_8:
			...
			rc1 = prefs_get_string(overwrite_str, 128, "stream defaults", "over_write_complete");
			if (!rc1 || !strcmp(overwrite_str, "larger")) {
				prefs_set_string("stream defaults", "over_write_complete", "version");
			}
			/* Fall through */
		case PREFS_VERSION_1_63_4:
			/* Silently update localhost to 127.0.0.1 */
			rc1 = prefs_get_string(localhost_str, SR_MAX_PATH, "wstreamripper", "localhost");
			...
			rc1 = prefs_get_string(codeset_str, MAX_CODESET_STRING, "stream defaults", "codeset_metadata");
			if (!rc1 || !strcmp(codeset_str, "UTF-8")) {
				prefs_set_string("stream defaults", "codeset_metadata", "ISO-8859-1");
			}
			rc1 = prefs_get_string(codeset_str, MAX_CODESET_STRING, "stream defaults", "codeset_relay");
			if (!rc1 || !strcmp(codeset_str, "UTF-8")) {
				prefs_set_string("stream defaults", "codeset_relay", "ISO-8859-1");
			}
...
void prefs_save(void)
{
	...
	keyfile_contents = g_key_file_to_data(m_key_file, &keyfile_contents_len, &error);
	if (error) {
...
void prefs_get_global_prefs(GLOBAL_PREFS * global_prefs)
{
	...
	prefs_get_string(global_prefs->version, MAX_VERSION_LEN, "sripper", "version");
...
void prefs_set_global_prefs(GLOBAL_PREFS * global_prefs)
{
	...
	prefs_set_stream_prefs_keyfile(&global_prefs->stream_prefs, 0, "stream defaults");
}

void prefs_get_wstreamripper_prefs(WSTREAMRIPPER_PREFS * prefs)
{
	int i, p;
...
void prefs_set_wstreamripper_prefs(WSTREAMRIPPER_PREFS * prefs)
{
	int i, p;
	char *group = "wstreamripper";
...
static void prefs_get_ushort(u_short * dest, char *group, char *key)
{
	GError *error = NULL;
	gint value;
...
static void prefs_set_stream_prefs_keyfile(STREAM_PREFS * prefs, STREAM_PREFS * gp, char *group)
{
	if (!m_key_file)
		return;
	...
	/* Overwrite */
	g_key_file_set_string(m_key_file, group, "over_write_complete", overwrite_opt_to_string(prefs->overwrite));

	/* Flags */
	prefs_set_integer(group, "auto_reconnect", OPT_FLAG_ISSET(prefs->flags, OPT_AUTO_RECONNECT));
	prefs_set_integer(group, "separate_dirs", OPT_FLAG_ISSET(prefs->flags, OPT_SEPARATE_DIRS));
	prefs_set_integer(group, "make_relay", OPT_FLAG_ISSET(prefs->flags, OPT_MAKE_RELAY));
	prefs_set_integer(group, "add_numeric_prefix", OPT_FLAG_ISSET(prefs->flags, OPT_COUNT_FILES));
	prefs_set_integer(group, "add_date_stamp", OPT_FLAG_ISSET(prefs->flags, OPT_DATE_STAMP));
	prefs_set_integer(group, "check_max_bytes", OPT_FLAG_ISSET(prefs->flags, OPT_CHECK_MAX_BYTES));
	prefs_set_integer(group, "keep_incomplete", OPT_FLAG_ISSET(prefs->flags, OPT_KEEP_INCOMPLETE));
	prefs_set_integer(group, "rip_single_file", OPT_FLAG_ISSET(prefs->flags, OPT_SINGLE_FILE_OUTPUT));
	prefs_set_integer(group, "truncate_incomplete_when_in_complete",
			  OPT_FLAG_ISSET(prefs->flags, OPT_TRUNCATE_DUPS));
	prefs_set_integer(group, "rip_individual_tracks", OPT_FLAG_ISSET(prefs->flags, OPT_INDIVIDUAL_TRACKS));
	prefs_set_integer(group, "use_ext_cmd", OPT_FLAG_ISSET(prefs->flags, OPT_EXTERNAL_CMD));
	prefs_set_integer(group, "add_id3v1", OPT_FLAG_ISSET(prefs->flags, OPT_ADD_ID3V1));
	prefs_set_integer(group, "add_id3v2", OPT_FLAG_ISSET(prefs->flags, OPT_ADD_ID3V2));

	/* Splitpoint options */
	prefs_set_integer(group, "xs", prefs->sp_opt.xs);
	prefs_set_integer(group, "xs_offset", prefs->sp_opt.xs_offset);
	prefs_set_integer(group, "xs_silence_length", prefs->sp_opt.xs_silence_length);
	prefs_set_integer(group, "xs_search_window_1", prefs->sp_opt.xs_search_window_1);
	prefs_set_integer(group, "xs_search_window_2", prefs->sp_opt.xs_search_window_2);
