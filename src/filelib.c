...
parse_and_subst_dir(RIP_MANAGER_INFO * rmi,
		    gchar * pattern_head, gchar * pattern_tail, gchar * opat_path, int is_for_showfile);
static void
parse_and_subst_pat(RIP_MANAGER_INFO * rmi,
		    gchar * newfile, TRACK_INFO * ti, gchar * directory, gchar * pattern, gchar * extension);
static void set_default_pattern(RIP_MANAGER_INFO * rmi, BOOL get_separate_dirs, BOOL do_count);
static error_code
set_output_directory(RIP_MANAGER_INFO * rmi,
		     ...
		     gchar * default_pattern_tail, int get_separate_dirs, int get_date_stamp, int is_for_showfile);
...
static void fill_date_buf(RIP_MANAGER_INFO * rmi, gchar * datebuf, int datebuf_len);
...
static void
filelib_rename_versioned(gchar ** new_fn, RIP_MANAGER_INFO * rmi, gchar * directory, gchar * fnbase, gchar * extension);

/*****************************************************************************
 * Public Functions
 *****************************************************************************/
error_code filelib_init(RIP_MANAGER_INFO * rmi, BOOL do_individual_tracks, BOOL do_count, int count_start, BOOL keep_incomplete, BOOL do_show_file, int content_type, char *output_directory,	/* Locale encoded - from command line */
			...
			int get_separate_dirs, int get_date_stamp, char *icy_name)
{
	...
	gstring_from_string(rmi, tmp_output_directory, SR_MAX_PATH, output_directory, CODESET_LOCALE);
	debug_printf("converting output_pattern\n");
	gstring_from_string(rmi, tmp_output_pattern, SR_MAX_PATH, output_pattern, CODESET_LOCALE);
	debug_printf("converting showfile_pattern\n");
	gstring_from_string(rmi, tmp_showfile_pattern, SR_MAX_PATH, showfile_pattern, CODESET_LOCALE);
	debug_printf("converting icy_name\n");
	gstring_from_string(rmi, fli->m_icy_name, SR_MAX_PATH, icy_name, CODESET_METADATA);
	...
	set_output_directory(rmi,
			     ...
			     fli->m_default_pattern, m_("%A - %T"), get_separate_dirs, get_date_stamp, 0);
	...
	if (do_show_file) {
		...
		set_output_directory(rmi,
				     ...
				     tmp_output_directory,
				     fli->m_default_showfile_pattern, m_(""), get_separate_dirs, get_date_stamp, 1);
...
error_code filelib_start(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti)
{
	...
	msnprintf(newfile, TEMP_STR_LEN, m_S m_S m_S, fli->m_incomplete_directory, fnbase, fli->m_extension);
	if (fli->m_keep_incomplete) {
		filelib_rename_versioned(0, rmi, fli->m_incomplete_directory, fnbase, fli->m_extension);
	}
...
error_code
filelib_end(RIP_MANAGER_INFO * rmi, TRACK_INFO * ti, enum OverwriteOpt overwrite, BOOL truncate_dup, gchar * fullpath)
{
	FILELIB_INFO *fli = &rmi->filelib_info;
	...
	parse_and_subst_pat(rmi, new_path, ti, fli->m_output_directory, fli->m_output_pattern, fli->m_extension);
	...
	if (do_count) {
		if (fli->m_count < 0) {
			...
			msnprintf(&fli->m_default_pattern[mstrlen(fli->m_default_pattern)],
				  SR_MAX_PATH - mstrlen(fli->m_default_pattern), m_("%%%dq_"), fli->m_count);
		}
	}
		     ...
		     gchar * default_pattern_tail, int get_separate_dirs, int get_date_stamp, int is_for_showfile)
{
	...
	if (output_directory && *output_directory) {
		...
		debug_printf("devicesplit: %d -> %d %d\n",
			     mstrlen(output_directory), mstrlen(odir_device), mstrlen(odir_path));
	}
	...
	msnprintf(pattern_head, SR_MAX_PATH, m_S m_S m_S, device, cwd_path, odir_path);
	debug_printf("Composed pattern head (%d) <- (%d,%d,%d)\n",
		     mstrlen(pattern_head), mstrlen(device), mstrlen(cwd_path), mstrlen(odir_path));
	parse_and_subst_dir(rmi, pattern_head, pattern_tail, opat_path, is_for_showfile);
		    ...
		    gchar * pattern_head, gchar * pattern_tail, gchar * opat_path, int is_for_showfile)
{
	...
	while (phi < SR_MAX_BASE) {
		...
		case m_('S'):
			/* append stream name */
			mstrncpy(&pattern_head[phi], fli->m_stripped_icy_name, SR_MAX_BASE - phi);
			phi = mstrlen(pattern_head);
			opi += 2;
			continue;
		case m_('d'):
			/* append date info */
			mstrncpy(&pattern_head[phi], fli->m_session_datebuf, SR_MAX_BASE - phi);
			phi = mstrlen(pattern_head);
			opi += 2;
			continue;
		case m_('0'):
		    ...
		    gchar * newfile, TRACK_INFO * ti, gchar * directory, gchar * pattern, gchar * extension)
{
	...
	while (nfi < MAX_FILEBASELEN) {
		...
		case m_('q'):
			...
			msnprintf(temp, DATEBUF_LEN, m_("%04d"), get_next_sequence_number(rmi, newfile));
...
static long get_file_size(RIP_MANAGER_INFO * rmi, gchar * filename)
{
	FILE *fp;
	long len;
	char fn[SR_MAX_PATH];
...
static BOOL new_file_is_better(RIP_MANAGER_INFO * rmi, gchar * oldfile, gchar * newfile)
{
	...
	debug_printf("NFB:oldfilesize = %li, newfilesize = %li, " "overwriting file\n", oldfilesize, newfilesize);
	return TRUE;
}
...
#else
	close(open(fn, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH));
#endif
}
...
static void filelib_adjust_cuefile(RIP_MANAGER_INFO * rmi, gchar * new_show_name, gchar * new_cue_name)
{
	gchar *tmp_fn;
	gint fd;
	FILE *fp_in, *fp_out;
...
static error_code filelib_open_showfiles(RIP_MANAGER_INFO * rmi)
{
	...
	parse_and_subst_pat(rmi, fli->m_cue_name, 0, fli->m_showfile_directory, fli->m_showfile_pattern, m_(".cue"));
	...
	trim_mp3_suffix(rmi, new_fnbase);
	filelib_rename_versioned(&new_show_name, rmi, new_dir, new_fnbase, fli->m_extension);
	filelib_rename_versioned(&new_cue_name, rmi, new_dir, new_fnbase, m_(".cue"));
