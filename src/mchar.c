 ...
 * Lstring functions
 *****************************************************************************/
void lstring_initialize(Lstring * lstring)
{
	lstring->num_bytes = 0;
	lstring->data = 0;
}
 ...
 *****************************************************************************/
void gmem_concat(gchar ** base_mem,	/* In/Out: The memory to be concatenated to */
...
void convert_string_with_replacement(gchar ** output_string,	/* Output: The converted string */
	...
	*output_bytes = 0;
	...
	repl_string = g_convert(repl, strlen(repl), to_codeset, "UTF-8", &br, &bw, &error);
	...
	while (cur < input_bytes) {
		...
		br = 0;
		os = g_convert_with_iconv(&input_string[cur], bytes_to_convert, giconv, &br, &bw, &error);
...
int utf8cpy(gchar * dst, gchar * src, int dst_len)
{
	gchar *s = src;
	gchar *d = dst;
	gunichar c;
...
void lstring_from_lstring(Lstring * lstring_out,	/* Output: Output string */
			  Lstring * lstring_in,	/* Input: Input string */
			  char *from_codeset,	/* Input: Codeset of input string */
			  char *to_codeset)
{				/* Input: Codeset of output string */
	convert_string_with_replacement(&lstring_out->data,
					&lstring_out->num_bytes,
					lstring_in->data, lstring_out->num_bytes, from_codeset, to_codeset, "?");
}

void lstring_from_gstring(Lstring * lstring_out,	/* Output: Output string */
			  gchar * gstring_in,	/* Input: Input string */
			  char *to_codeset)
{				/* Input: Codeset of output string */
	convert_string_with_replacement(&lstring_out->data,
					&lstring_out->num_bytes,
					gstring_in, strlen(gstring_in) + 1, "UTF-8", to_codeset, "?");
}
	...
	*m = 0;
...

	{
		...
		/* GCS FIX: This is not correct, as strlen(c) won't work 
		   for UTF-16. */
		convert_string_with_replacement(&gstring, &gstring_len, c, strlen(c) + 1, src_codeset, "UTF-8", "?");
		if (!gstring) {
...
int string_from_gstring(RIP_MANAGER_INFO * rmi, char *c, int clen, mchar * m, int codeset_type)
{
	...
	*c = 0;
		...
		return 0;
	{
		...
		convert_string_with_replacement(&cstring, &cstring_len, m, strlen(m) + 1, "UTF-8", tgt_codeset, "?");
		if (!cstring) {
...
const char *default_codeset(void)
{
...
int is_id3_unicode(RIP_MANAGER_INFO * rmi)
{
	CODESET_OPTIONS *mchar_cs = &rmi->mchar_cs;
